...
現在掲載している価格は、2026年8月時点の店頭掲示（ショーケースの品書き）にもとづいています。
...
> 現在 `hero-yakitori-tare.jpg` と `img-` で始まる7点がイメージ写真です。
> 店舗の実写が用意できたら、`data/photos.ts` の該当箇所を差し替えてください。
> `kind: 'image'` で検索すれば差し替え対象がすべて見つかります。
